...
One thing to note is that we highly recommend the use of the tool such as [NGSCheckMate](https://github.com/parklab/NGSCheckMate) for verifying that (human) samples come from the same individual or not. This is useful both for ensuring that the enriched and non-enriched samples do indeed come from the same individual, but also to check that paired samples are actually paired (for instance when comparing pre- and post-treatment).
...
Note, we highly recommend using the hg38/GRCh38 reference genome rather than hg19 as used here!
...
* Calculate the average CG density of each window, using the `fragmentLength` and `fragmentSD` parameters to define the expected contribution of fragments to the likelihood that they overlap the window (see the Calculating CpG Density section below). 
...
However, we note that these considerations might not apply for everyone; the `qsea::addLibraryFactors()` function may be used to apply this normalisation if required.
