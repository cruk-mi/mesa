test_that("Annotation getting works", {
  ...
  expect_no_error(plotRegionsHeatmap(exampleTumourNormal %>% mutate(group = tumour),
    regionsToOverlap = regions, sampleAnnotation = c(tumour, type),
    annotationColors = list(tumour = c("Normal" = "blue", "Tumour" = "firebrick4"))
  ))

  expect_no_error(plotRegionsHeatmap(exampleTumourNormal %>% mutate(group = tumour),
    regionsToOverlap = regions, sampleAnnotation = c(tumour, type, age),
    annotationColors = list(tumour = c("Normal" = "blue", "Tumour" = "firebrick4"))
  ))
...
test_that("Testing hg38 related annotation/plotting functions", {
  ...
  testPlotGeneHeatmap(exampleTumourNormal %>% mutate(group = tumour),
    gene = "HOXA10", sampleAnnotation = c(tumour, type),
    annotationColors = list(tumour = c("Normal" = "blue", "Tumour" = "firebrick4"))
  )
  ...
  testPlotGeneHeatmap(exampleTumourNormal,
    gene = "HOXA10", normMethod = "nrpm", maxScale = 3,
...
test_that("Testing general functionality", {
  randomSet <- qsea::getExampleQseaSet(repl = 8, expSamplingDepth = 1000000) %>%
    mutate(
      patient = stringr::str_remove(sample_name, "[TN]$"),
