...
pivotDMRsLonger <- function(DMRtable, FDRthres = 0.05, makePositive = FALSE) {
    pivotedDMRs <- DMRtable %>%
        dplyr::rename_with(
            ~ stringr::str_replace(.x, "_adjPval", ":adjPval")
        ) %>%
        dplyr::rename_with(
            ~ stringr::str_replace(.x, "_log2FC", ":log2FC")
        ) %>%
        dplyr::rename_with(
            ~ stringr::str_replace(.x, "_deltaBeta", ":deltaBeta")
        ) %>%
    ...
    if (makePositive) {
        pivotedDMRs <- pivotedDMRs %>%
            dplyr::mutate(
                group1new = ifelse(log2FC < 0, group2, group1),
...
#'
#' @export
summariseDMRsByContrast <- function(
    DMRtable, FDRthres = 0.05, log2FCthres = 0, deltaBetaThres = 0
) {
    ...
    contrastsDF %>%
        ...
        dplyr::mutate(
            nDown = tidyr::replace_na(nDown, 0),
...
summariseDMRsByGene <- function(DMRtable) {
    if (!("ENSEMBL" %in% colnames(DMRtable))) {
        stop(
            "Please call annotateWindows first on the",
            " DMRs prior to summariseDMRsByGene."
        )
    }
...
writeDMRsToExcel <- function(dataTable, path, FDRthres = 0.05) {
    ...
    purrr::walk(contrasts, function(x) {
        ...
        openxlsx::addFilter(
            wb_DMR, sheetName, row = 1,
            cols = seq_len(ncol(dataTable))
        )
...
writeDMRsToBed <- function(dataTable, folder, FDRthres = 0.05) {
    ...
    if (length(contrasts) == 0) {
        stop(
            "No columns ending in adjPval found!",
            " Is this a set of DMRs generated by calculateDMRs?"
        )
    }
    ...
    purrr::walk(contrasts, function(x) {
        ...
        dataTable %>%
            ...
            rtracklayer::export.bed(
                file.path(folder, paste0(contrastName, ".bed"))
            )
    })
...
#' @export
sliceDMRs <- function(
    DMRs, n = 1, metric = deltaBeta, makePositive = TRUE, FDRthres = 0.05
) {
    n <- as.integer(n)
    ...
    if (deparse(substitute(metric)) %in% c("position")) {
        ...
        message(glue::glue(
            "Choosing the windows with the smallest",
            " value of {rlang::ensym(metric)}"
        ))
        out <- positiveDMRs %>%
