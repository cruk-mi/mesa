...
plotDimRed <- function(object,
  ...
  if (!("pcas" %in% names(object))) {
    stop("First argument should be a pca or umap object from getPCA")
  }
...
getDimRed <- function(qseaSet,
    ...
    dplyr::group_map(.keep = TRUE, .f = function(sdGp, sdName) {
      ...
      purrr::pmap(sdGp, function(pcaName, windowSdName, topVarNum, topVarSamples, topVarNumInput, topVarSamplesInput, inputChanged) {
        ...
        if (method == "UMAP") {
          ...
          return(list(prcompObj =  list(x = uwotObj, windows = rownames(dataTable)), th = th))
  ...
  if (method == "UMAP") {
    windows <- purrr::map(pcas, "windows")
  } else if (method == "PCA") {
