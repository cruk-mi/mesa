...
#' @param nNeighbours Number of neighbours (for method UMAP only).
#' @param minDist Minimum distance between points (for method UMAP only).
#' @param returnDataTable A logical value indicating if the table of normalised values (generated by [getDataTable()] or by processing the input `dataTable`) should be returned as part of the output. See below for details.
...
#' @return A list of ggplot objects: one for each combination of `object$pcas` or `object$umaps`, `colour` and `components`.
#' @export
#'
plotDimRed <- function(object,
  ...
  if (!("res" %in% names(object)) ) {
    stop("First argument should be a pca or umap object from getPCA/getUMAP.")
  }
