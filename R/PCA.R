...
#' \item{res}{A named list of objects of class prcomp. List names correspond to entries in the `params$topVar` component of the output.}
...
#' @param scale A logical value indicating if windows should be scaled to have unit variance. Default is FALSE.
#' @param minDist The effective minimum distance between embedded points. Smaller values will result in a more clustered/clumped embedding, while larger values will result in a more even dispersal of points. See [uwot::umap()] documentation for further details (`min_dist` argument).
...
#' @return A list with the following components:
#' \item{res}{A named list where each componenet is a matrix of coordinates. List names correspond to entries in the `params$topVar` component of the output.}
#' \item{samples}{Names of the samples analysed.}
#' \item{windows}{A list of names of the windows analysed. List names correspond to entries in the `params$topVar` component of the output.}
#' \item{dataTable}{If `returnDataTable = TRUE`, the table of normalised values. The table will contain windows after filtering based on `regionsToOverlap` and `minDensity`, but prior to filtering based on `topVarNum`. Windows with missing values will also have been removed. The table will also include column(s) of window standard deviations, where applicable, with column name(s) corresponding to entries in the `params$topVar` component of the output.}
#' \item{params}{A list of the parameters used.}
...
#' @param nUC Number of UMAP components to be calculated (for method UMAP only). Default is 2.
#' @param nNeighbours The size of local neighborhood (in terms of number of neighboring sample points) used for manifold approximation (for method UMAP only). Larger values result in more global views of the manifold, while smaller values result in more local data being preserved. In general values should be in the range 2 to 100.
#' @param minDist The effective minimum distance between embedded points (for method UMAP only). Smaller values will result in a more clustered/clumped embedding, while larger values will result in a more even dispersal of points. See [uwot::umap()] documentation for further details.
#' @param returnDataTable A logical value indicating if the table of normalised values (generated by [getDataTable()] or by processing the input `dataTable`) should be returned as part of the output. See below for details.
#' @param ... Other parameters to pass to [uwot::umap()] function.
#' @return A list with the following components:
#' \item{res}{A named list of objects of class prcomp for method PCA or a matrix of coordinates for method UMAP. List names correspond to entries in the `params$topVar` component of the output.}
#' \item{samples}{Names of the samples analysed.}
#' \item{windows}{A list of names of the windows analysed. List names correspond to entries in the `params$topVar` component of the output.}
#' \item{dataTable}{If `returnDataTable = TRUE`, the table of normalised values. The table will contain windows after filtering based on `regionsToOverlap` and `minDensity`, but prior to filtering based on `topVarNum`. Windows with missing values will also have been removed. The table will also include column(s) of window standard deviations, where applicable, with column name(s) corresponding to entries in the `params$topVar` component of the output.}
#' \item{params}{A list of the parameters used.}
...
plotDimRed <- function(object,
  ...
  if (!("res" %in% names(object)) ) {
    stop("First argument should be the output from getPCA()/getUMAP().")
  }
