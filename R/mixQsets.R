...
#' @export
mixSamples <- function(
    qseaSet, sample1, sample2,
    nReadsTotal, proportion,
    newName = NULL, groupName = NULL,
    onlyNew = FALSE,
    renormalise = TRUE
) {
    ...
    if (proportion < 0 || proportion > 1) {
        stop(glue::glue(
            "Proportion must be a number between 0 and 1,",
            " not {proportion}."
        ))
    }

    validFragNums <- qseaSet@libraries$file_name[
        c(sample1, sample2), "valid_fragments", drop = FALSE
    ] %>%
    ...
    message(glue::glue(
        "Mixing {nReads1} reads from {sample1}",
        " with {nReads2} reads from {sample2}."
    ))
    ...
    if (sampleColSums[sample1] < nReads1 * onTargetFrac1) {
        stop(glue::glue(
            "Not enough reads in {sample1},",
            " only {sampleColSums[sample1]} out of",
            " {nReads1 * onTargetFrac1} requested."
        ))
    }

    if (sampleColSums[sample2] < nReads2 * onTargetFrac2) {
        stop(glue::glue(
            "Not enough reads in {sample2},",
            " only {sampleColSums[sample2]} out of",
            " {nReads2 * onTargetFrac2} requested."
        ))
    }

    windows1 <- sample(
        rep(seq_len(nrow(counts)), counts[, sample1]),
        replace = FALSE, size = nReads1 * onTargetFrac1
    )
    windows2 <- sample(
        rep(seq_len(nrow(counts)), counts[, sample2]),
        replace = FALSE, size = nReads2 * onTargetFrac2
    )
    ...
    newCounts <- c(windows1, windows2) %>%
        ...
        dplyr::left_join(
            tibble::tibble(window = seq_len(nrow(counts))),
            ., copy = TRUE, by = "window"
        ) %>%
    ...
    GenomicRanges::mcols(newSet@cnv) <-
        proportion * reducedCNVMat[, sample1] +
        (1 - proportion) * reducedCNVMat[, sample2]
    colnames(GenomicRanges::mcols(newSet@cnv)) <- newName

    newSet@zygosity <-
        proportion * qseaSet@zygosity[sample1, , drop = FALSE] +
        (1 - proportion) * qseaSet@zygosity[sample2, , drop = FALSE]
...
#'
#' @keywords internal
mixThreeQsetSamples <- function(
    qseaSet, sample1, sample2, sample3,
    nReadsTotal, proportion1, proportion2,
    newName = NULL, groupName = NULL,
    onlyNew = FALSE,
    renormalise = TRUE
) {
    ...
    if (is.null(newName)) {
        newName <- paste0(
            "Mix", "_", sample1, "_", sample2, "_",
            sample3, "_", proportion1, "_", proportion2
        )
    }
    ...
    if (proportion1 < 0 || proportion1 > 1) {
        stop(glue::glue(
            "Proportion1 must be a number between 0 and 1,",
            " not {proportion1}."
        ))
    }
    ...
    if (proportion2 < 0 || proportion2 > 1) {
        stop(glue::glue(
            "Proportion2 must be a number between 0 and 1,",
            " not {proportion2}."
        ))
    }
    ...
    if (proportion1 + proportion2 >= 1) {
        stop(glue::glue(
            "Sum of proportions must be less than 1,",
            " not {proportion1 + proportion2}."
        ))
    }

    validFragNums <- qseaSet@libraries$file_name[
        c(sample1, sample2, sample3), "valid_fragments", drop = FALSE
    ] %>%
        tibble::rownames_to_column() %>%
        tibble::deframe()
    ...
    message(glue::glue(
        "Mixing {nReads1} reads from {sample1}",
        " with {nReads2} reads from {sample2}",
        " and {nReads3} reads from {sample3}."
    ))
    ...
    if (sampleColSums[sample3] < nReads3 * onTargetFrac2) {
        stop(glue::glue(
            "Not enough reads in {sample3},",
            " only {sampleColSums[sample3]} out of",
            " {nReads3 * onTargetFrac3} requested."
        ))
    }
    ...
    windows3 <- sample(
        rep(seq_len(nrow(counts)), counts[, sample3]),
        replace = FALSE, size = nReads3 * onTargetFrac3
    )
    ...
    GenomicRanges::mcols(newSet@cnv) <-
        proportion1 * reducedCNVMat[, sample1] +
        proportion2 * reducedCNVMat[, sample2] +
        (1 - proportion1 - proportion2) * reducedCNVMat[, sample3]
    colnames(GenomicRanges::mcols(newSet@cnv)) <- newName

    newSet@zygosity <-
        proportion1 * qseaSet@zygosity[sample1, , drop = FALSE] +
        proportion2 * qseaSet@zygosity[sample2, , drop = FALSE] +
        (1 - proportion1 - proportion2) *
        qseaSet@zygosity[sample3, , drop = FALSE]
    rownames(newSet@zygosity) <- newName
