...
getDataTable <- function(qseaSet, normMethod = "nrpm", useGroupMeans = FALSE, minEnrichment = 3, addMethodSuffix = FALSE, verbose = TRUE) {
    ...
    if (useGroupMeans) {
        if (verbose) {
            message(glue::glue("Generating table of {normMethod} values for {qseaSet %>% qsea::getRegions() %>% length()} regions across {qseaSet %>% getSampleGroups2() %>% length()} sample groups."))
        }
        tab <- qseaSet %>%
            qsea::makeTable(
        ...
        if (verbose) {
            message(glue::glue("Generating table of {normMethod} values for {qseaSet %>% qsea::getRegions() %>% length()} regions across {qseaSet %>% getSampleGroups2() %>% length()} samples."))
        }
        tab <- qseaSet %>%
            qsea::makeTable(
