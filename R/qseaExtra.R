...
#' @param centre A logical value indicating if windows should be centred to have mean zero. Default is TRUE.
...
#' \item{pcaObj}{A named list of objects of class prcomp. List names correspond to entries in the `params$topVar` component of the output.}
#' \item{samples}{Names of the samples analysed.}
#' \item{windows}{Names of the windows analysed, or a list of names if more than one PCA is performed.}
...
getPCA <- function(qseaSet, 
  ...
  pca <- topVar %>% 
    ...
    dplyr::group_map(.keep = TRUE, .f = function(sdGp, sdName) {
      ...
      purrr::pmap(sdGp, function(pcaName, windowSdName, topVarNum, topVarSamples, ...) {
        ...
        pcaObj <- dataTable %>% 
          prcomp(center = centre, scale. = scale, rank. = nPC)
        
        return(list(pcaObj = pcaObj, th = th))
  ...
  pcaObj <- purrr::map(pca, "pcaObj")
  
  windows <- purrr::map(pcaObj, ~ rownames(.x$rotation))
    
  return(list(pcaObj = pcaObj, 
              samples = samples,
              windows = windows,
