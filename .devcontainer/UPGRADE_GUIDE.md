...
Everything else (check-bioc.yml, Dockerfile, package lists, install.R,
build-image.yml) reads from `versions.env` automatically - you should not need to
edit them.

> **Note:** the slim devcontainer build falls back to source installs via
> `install.R` for any R/Bioc release other than the one packaged on conda (the
> Dockerfile only uses the conda package list for the exact version pair it was
> built against). New releases therefore build from source until conda catches
> up - slower, but no manual change is required.
...
## Step 6 - Update CI workflow (no edit needed)

`check-bioc.yml` no longer hardcodes the R version or Bioc Docker image tag.
A `read-versions` pre-job parses `versions.env` and feeds the R version,
Bioc version, and `RELEASE_X_XX` Docker tag into the build matrix, so the
workflow picks up the new versions automatically once Step 5 is done.

```bash
# Verify the workflow still reads from versions.env (no manual change required)
grep -nE "read-versions|needs.read-versions.outputs" .github/workflows/check-bioc.yml
```

> **RSPM Ubuntu codename:** the only value still hardcoded in the matrix is the
> `rspm` URL codename (`jammy`). It tracks the Bioc Docker base image's Ubuntu
> version, not `versions.env`. Only change it if the Bioc Docker base image
> changed Ubuntu version (focal = 20.04, jammy = 22.04, noble = 24.04).
