...
| `ggtree`, `immunedeconv` and immunedeconv's `Remotes:` (xCell, EPIC, MCPcounter, mMCPcounter, ConsensusTME) | explicit GitHub SHAs in `install_github.R` | — | pinned on purpose; re-check the `Remotes:` pins when bumping immunedeconv |
...
`Config/roxygen2/version` in `DESCRIPTION` records which roxygen2 generated the committed
`man/` and `NAMESPACE`. (roxygen2 8.0.0 renamed this field from `RoxygenNote`; the resolver
still reads the old name so older branches keep working.)

`resolve_versions.sh` emits it as `ROXYGEN_VERSION`, `install.R` installs that exact
version, and the `roxygen-drift` CI job regenerates with it and fails on any diff. So
**upgrading roxygen2 is a one-line edit to `DESCRIPTION`** — the same story as R.

Do it in its own PR, never inside an unrelated change: a roxygen upgrade typically rewrites
most man pages, and that diff must be reviewable as the mechanical change it is.
...
| `man/*.Rd` | Regenerated by `roxygen2::roxygenise()` — never hand-edit | only if roxygen comments changed in Step 5 |
