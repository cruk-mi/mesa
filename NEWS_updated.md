...
### CHANGES
* Converted `plotPCA` into a submethod for the `qsea` defined method. [!14](https://github.com/cruk-mi/mesa/pull/14)
* `plotPCA` gains a `verbose` option to turn off most of the messages produced. [!36](https://github.com/cruk-mi/mesa/pull/36)
...
* `getSampleTable` is now defined for PCA/UMAP objects. [!36](https://github.com/cruk-mi/mesa/pull/36)
...
* `calculateDMRs` now correctly returns an empty data frame if no DMRs are found. [!53](https://github.com/cruk-mi/mesa/pull/53)

# mesa 0.4.1

### BUG FIXES
* `plotPCA` now works when colouring by a factor variable [!40](https://github.com/cruk-mi/mesa/pull/40)
* `mutate` now cannot be used to change the sample_name in the sample table of a qseaSet which breaks the object. `renameQsetNames` or `renameSamples` must be used for this. [!10](https://github.com/cruk-mi/mesa/pull/10)
